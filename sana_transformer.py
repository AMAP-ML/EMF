...
class SanaTransformer2DModel(ModelMixin, ConfigMixin, PeftAdapterMixin, FromOriginalModelMixin):
    ...
    def __init__(
        ...
        if guidance_embeds:
            ...
            self.time_embed_t = SanaCombinedTimestepGuidanceEmbeddings(inner_dim)
            self.time_embed_r = SanaCombinedTimestepGuidanceEmbeddings(inner_dim)

        else:
            self.time_embed = AdaLayerNormSingle(inner_dim)
            self.time_embed_t = AdaLayerNormSingle(inner_dim)
            self.time_embed_r = AdaLayerNormSingle(inner_dim)
    ...
    # def tie_time_embeds(self):
    #     self.time_embed_t.load_state_dict(self.time_embed.state_dict())
    #     self.time_embed_r.load_state_dict(self.time_embed.state_dict())
    @property
    ...
    def forward(
        ...
        timestep: Optional[torch.Tensor] = None,   # 使用flow mathcing
        timestep_t: Optional[torch.Tensor] = None,   # 当前时间 t 使用mean flow
        timestep_r: Optional[torch.Tensor] = None,   # 参考时间 r 使用mean flow
        ...
        elif timestep_t is not None:
            # print('ours_time_embedd')
            
            timestep_r, embedded_timestep_r = self.time_embed_r(
                        timestep_t-timestep_r, batch_size=batch_size, hidden_dtype=hidden_states.dtype
                    )
            timestep_t, embedded_timestep_t = self.time_embed_t(
                        timestep_t, batch_size=batch_size, hidden_dtype=hidden_states.dtype
                    )
            
        
        else:
             raise ValueError("None timestep!") 
        ...
        if timestep is not None:
            ...
            # print(f"hidden_states end: {hidden_states}")
                ...
                # 2. Transformer blocks
            # print('ours_time_embedd_transformer')
            if torch.is_grad_enabled() and self.gradient_checkpointing:
                for index_block, block in enumerate(self.transformer_blocks):
                    hidden_states = self._gradient_checkpointing_func(
                        block,
        ...
        if timestep is not None:
            ...
            # print('ours_time_embedd_transformer_back')
            hidden_states = self.norm_out(hidden_states, embedded_timestep_t+embedded_timestep_r, self.scale_shift_table)

        # print(f"hidden_states_norm: {hidden_states}")
