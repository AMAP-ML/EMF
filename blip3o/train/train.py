...
def train():
    ...
    if model_args.vision_tower is None:
        ...
        # # # sana预训练，全部参数都训练
        # print('zcx_38M_zcx_38M_zcx_38M_zcx_38M_zcx_38M_zcx_38M_zcx_38M_zcx_38M')
        # for name, param in model.named_parameters():
        #     if "model.sana." in name:
        #         param.requires_grad_(True)
        #     else:
        #         param.requires_grad_(False)

        # for name, param in model.named_parameters(): # v2 弃用
        #     if "sana" in name:

        #         if "transformer_blocks" or 'time_embed' or 'patch_embed' or 'proj_out' or 'caption' in name:
        #             param.requires_grad_(True)
        #         else:
        #             param.requires_grad_(False)
        ...
        # sana SFT，只训练sana transformer和time_embed
        for name, param in model.named_parameters():
            # print(name)
            if ("model.sana.transformer_blocks" in name) or ("model.sana.time_embed_" in name) or ('model.sana.scale_shift_table' in name):
                param.requires_grad_(True)
            else:
                param.requires_grad_(False)
    ...
    if list(pathlib.Path(training_args.output_dir).glob("checkpoint-*")):
        ...
        with torch.serialization.safe_globals([getattr, deepspeed.runtime.fp16.loss_scaler.LossScaler]):
            trainer.train(resume_from_checkpoint=True)
    else:
        trainer.train()
    trainer.save_state()
